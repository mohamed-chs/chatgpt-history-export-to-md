...
def main():
    ...
    deduplication_mode = config["deduplication_mode"] = qst.select(
    ...
    want_markdown = config["want_markdown"] = qst.confirm(
    ...
    if want_markdown:
        ...
        choices = [
            qst.Choice(header, checked=value)
            for header, value in config["yaml_headers"].items()
        ]

        selected_headers = qst.checkbox(
    ...
    want_wordclouds = config["want_wordclouds"] = qst.confirm(
    ...
    if want_wordclouds:
        font = config["data_viz"]["wordclouds"]["font"] = qst.select(
        ...
        colormap = config["data_viz"]["wordclouds"]["colormap"] = qst.select(
        ...
        all_months = config["data_viz"]["wordclouds"]["all_months"] = qst.confirm(
        ...
        if all_months:
            ...
            start_month = config["data_viz"]["wordclouds"]["start_month"] = qst.select(
                "Select a start month for the word cloud :",
                choices=month_names,
                default=config["data_viz"]["wordclouds"]["start_month"],
            ).ask()

            end_month = config["data_viz"]["wordclouds"]["end_month"] = qst.select(
